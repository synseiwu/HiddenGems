...
1. The hg-stripe-checkout Edge Function must be redeployed from supabase/functions/hg-stripe-checkout/index.ts
2. Stripe links in config.js must match the current mode:
   - PAYPAL_CLIENT_ID
...
3. Click Buy Access and complete Stripe checkout.
...
Sandbox vs live
- Sandbox: https://api-m.sandbox.stripe.com with sandbox app credentials
- Live: https://api-m.stripe.com with live app credentials
...
- The Stripe Edge Function now honors the siteUrl sent by the frontend, so Stripe can return to your real site even if SITE_URL was missing in Supabase secrets.
...
3. In Supabase Edge Functions, redeploy hg-stripe-checkout from supabase/functions/hg-stripe-checkout/index.ts.
...
- Typical flow:
  ...
  supabase secrets set PAYPAL_CLIENT_ID=... PAYPAL_CLIENT_SECRET=... PAYPAL_BASE_URL=https://api-m.stripe.com SITE_URL=https://hiddengems.space
  supabase functions deploy hg-stripe-checkout --no-verify-jwt


Current launch note:
- This package is using Stripe Payment Links for direct video and VIP checkout.
- Video unlocks depend on a signed-in user and the success page returning to Hidden Gems.
- Keep test links while testing; replace with live Stripe links before promotion.
