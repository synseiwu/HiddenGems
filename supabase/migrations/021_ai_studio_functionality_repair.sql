-- AI Studio functionality repair.
-- Fixes: Could not find the table 'public.ai_conversations' in the schema cache.
...
create table if not exists public.ai_settings (
  ...
  system_prompt text not null default 'You are AI Studio, a helpful assistant inside the platform. Be useful, clear, and safe.',
...
drop policy if exists "Admins can delete all AI conversations" on public.ai_conversations;
...
create policy "Admins can delete all AI conversations"
on public.ai_conversations
for delete
to authenticated
using (public.is_admin());
...
-- Reload PostgREST/Supabase schema cache so the frontend can see new tables immediately.
select pg_notify('pgrst', 'reload schema');

select
  'ai_conversations_exists' as check_name,
  to_regclass('public.ai_conversations')::text as result
union all
select
  'ai_messages_exists',
  to_regclass('public.ai_messages')::text
union all
select
  'ai_settings_exists',
  to_regclass('public.ai_settings')::text;
