...
window.HiddenGemsApp = (() => {
  ...
  async function startVipCheckout() {
    ...
    if (!target || target.includes('REPLACE_WITH')) throw new Error('Add your real VIP Stripe subscription link in config.js.');
  ...
  function videoPrimaryAction(state, video) {
    ...
    return { text: `Buy Access ${moneyLabelFromVideo(video)}`, href: '#', kind: 'buy' };
  }
  ...
  function renderVideoPage() {
    ...
    const mount = async () => {
      ...
      if (!unlocked && state.role === 'guest' && video.access === 'guest') {
        banner.innerHTML = `<p class="text-sm uppercase tracking-[0.2em] text-pink-300">Guest Preview</p><h3 class="mt-2 text-2xl font-bold text-white">Preview before buying</h3><p class="mt-3 text-neutral-300">Guests can preview this title before purchase. Buy direct access for ${moneyLabelFromVideo(video)} to unlock the full video.</p>`;
        player.innerHTML = await renderPublicPreviewShell(video, `Preview only. Buy this title for ${moneyLabelFromVideo(video)} to unlock full playback.`);
        actionButton.textContent = `Buy Access ${moneyLabelFromVideo(video)}`; actionButton.href = '#'; actionButton.onclick = (event) => { event.preventDefault(); buyVideo(video); setTimeout(mount, 350); };
        return;
      }
      const source = getVideoSource(video);
  ...
  function renderVipCheckoutPage() {
    ...
    getState().then((state) => {
      ...
      else if (!STRIPE_VIP_SUBSCRIPTION_LINK) note.textContent = 'Add your Stripe VIP subscription link in config.js before testing checkout.';
      else note.textContent = 'Stripe subscription checkout is ready. This simple setup redirects customers to Stripe without using Supabase payment functions.';
    });
  ...
  function initPage() {
    ...
    if (key === 'about') { renderSimplePage('About Hidden Gems', 'About', '<p>Hidden Gems is structured around three access tiers: guest, VIP, and admin.</p><p>Guests buy standard videos directly with PayPal. VIP opens the VIP vault and download access. Admin can manage the site and access every title.</p>'); return; }
