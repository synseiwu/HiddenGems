...
window.HiddenGemsApp = (() => {
  ...
  function renderVideoPage() {
    ...
    const mount = async () => {
      ...
      if (!unlocked && state.role === 'guest' && video.access === 'guest') {
        banner.innerHTML = `<p class="text-sm uppercase tracking-[0.2em] text-pink-300">Guest Preview</p><h3 class="mt-2 text-2xl font-bold text-white">Preview before buying</h3><p class="mt-3 text-neutral-300">Guests can preview this title before purchase. Buy direct access using the PayPal link added for this video.</p>`;
  ...
  function renderAdminPage() {
    ...
    const mount = async () => {
      ...
      const populateVideoForm = (video) => {
        ...
        if (form.elements.priceCents) form.elements.priceCents.value = normalizePriceCents(video.priceCents);
      ...
      const resetVideoForm = () => {
        ...
        if (form.elements.priceCents) form.elements.priceCents.value = '0';
  ...
  bindRealtimeSync();
  return { storage, getState, getVideo, getCategory, allVideos, renderCategoryPage, renderVideoPage, renderPointsStore, renderVipCheckoutPage, renderLibraryPage, renderAllVideosPage, renderSimplePage, shellHeader, shellFooter, bindCommonUi, initialsFromEmail, mountSharedHeader, refreshHeaderUi, signOutUser, syncVipForCurrentUser, canAccessVideo, refreshSupabaseCategories, refreshSiteSettings, initVipCheckoutPage, startVipCheckout, startVideoCheckout, finalizeCheckoutFromUrl, finalizeStripeVideoCheckoutFromLocalStorage, updateCurrentUserProfile, getSupabaseClient, resetSupabaseSession, getSessionUser };
})();
