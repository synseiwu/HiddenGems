  ...
  function renderVideoPage() {
    ...
    const mount = async () => {
      ...
      if (!unlocked && state.role === 'guest' && video.access === 'guest') {
        banner.innerHTML = `<p class="text-sm uppercase tracking-[0.2em] text-pink-300">Purchase Needed</p><h3 class="mt-2 text-2xl font-bold text-white">Buy direct access for ${moneyLabelFromVideo(video)}</h3><p class="mt-3 text-neutral-300">This is a one-time purchase tied to your signed-in Hidden Gems account.</p>`;
        player.innerHTML = '<p class="text-neutral-400">Buy this title to unlock protected access for this account.</p>';
