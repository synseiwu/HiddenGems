...
window.HiddenGemsApp = (() => {
  ...
  async function getPurchasedVideoIds(state = null) {
    ...
    // SECURITY: Purchases must come from Supabase only. Never trust localStorage
    // for video access because visitors can edit localStorage in DevTools.
    if (supabase && user?.id) {
      try {
  ...
  async function unlockVideoForState(state, video, paymentMeta = {}) {
    // SECURITY: The browser is not allowed to grant purchases. Stripe/webhook,
    // an admin-only backend action, or manual Supabase entry must create access rows.
    console.warn('Client-side video unlock blocked. Waiting for verified Supabase purchase record.', { videoId: video?.id, paymentMeta });
    toast('Payment received. Access will appear after payment verification completes.', 'info');
  ...
  function getRoleForAccount(user, email, profile) {
    const normalized = String(email || user?.email || profile?.email || '').trim().toLowerCase();
    ...
    // SECURITY: Never trust browser/localStorage role overrides. Role/VIP access
    // must come from Supabase, protected by RLS and database triggers.
    const profileRole = String(profile?.role || '').trim().toLowerCase();
    if (profileRole === 'vip' || profile?.is_vip === true) return 'vip';
  ...
  async function syncVipForCurrentUser(isVip) {
    // SECURITY: The browser cannot grant VIP. Refresh from Supabase after Stripe
    // webhook/admin verification updates the protected profile row.
    await getProfile();
    window.dispatchEvent(new CustomEvent('hg:state-changed'));
    return false;
  }
  ...
  async function finalizeStripeVideoCheckoutFromLocalStorage() {
    ...
    try { window.localStorage.removeItem(PENDING_STRIPE_VIDEO_KEY); } catch (_) {}
    window.dispatchEvent(new CustomEvent('hg:state-changed'));
    return { status: 'pending_verification', videoUnlocked: false, title: video.title || pending?.title || '', videoId };
  }
  ...
  async function finalizeStripeVipCheckoutFromLocalStorage() {
    ...
    try { window.localStorage.removeItem(PENDING_STRIPE_VIP_KEY); } catch (_) {}
    window.dispatchEvent(new CustomEvent('hg:state-changed'));
    return { status: 'pending_verification', vipUnlocked: false, title: 'VIP Subscription' };
  }
  ...
  async function startVideoCheckout(video) {
    ...
    if (purchasedIds.has(String(video.id))) {
  ...
  function videoPrimaryAction(state, video) {
    ...
    const purchasedIds = new Set(Array.isArray(state?.purchasedIds) ? state.purchasedIds.map(String) : []);
    if (state.role === 'admin' || state.role === 'vip' || purchasedIds.has(String(video?.id || ''))) return { text: video.videoUrl ? 'Watch Now' : 'Open Video', href: `video.html?id=${video.id}`, kind: 'link' };
  ...
  function renderCategoryPage(slug) {
    ...
    const mount = async () => {
      ...
      state.purchasedIds = await getPurchasedVideoIds(state);
  ...
  function renderAdminPage() {
    ...
    const mount = async () => {
      ...
      const renderRoleList = () => {
        document.getElementById('admin-role-list').innerHTML = '<p class="text-sm text-neutral-400">Role overrides are disabled for security. Admin access is allowlist-only, and VIP access must come from verified payment records.</p>';
      };
      ...
      document.getElementById('admin-role-form').addEventListener('submit', (event) => {
        event.preventDefault();
        toast('Role changes are locked down. Manage admins through the Supabase admin allowlist and VIP through verified Stripe/payment records only.', 'error');
