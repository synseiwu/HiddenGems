...
window.HiddenGemsApp = (() => {
  ...
  function categoryDisplayName(slug, fallback = '') {
    return TEMP_CATEGORY_LABELS[String(slug || '').trim()] || String(fallback || '').trim() || titleFromSlug(slug);
  }
  ...
  async function saveVideoToSupabase(item) {
    ...
    const resolvedCategorySlug = String(item.categorySlug || 'category-1').trim() || 'category-1';
    const resolvedCategoryTitle = categoryDisplayName(resolvedCategorySlug, item.categoryTitle || '');
    const payload = {
      id: item.id,
      title: item.title,
  ...
  function renderVideoPage() {
    ...
    const mount = async () => {
      ...
      if (!unlocked && state.role === 'guest' && video.access === 'guest') {
        ...
        player.innerHTML = '<p class="text-neutral-400">Unlock this title to watch a protected preview.</p>';
        actionButton.textContent = `Unlock for ${video.points} pts`; actionButton.href = '#'; actionButton.onclick = (event) => { event.preventDefault(); buyVideo(video); setTimeout(mount, 350); };
        return;
      }
      const source = getVideoSource(video);
      banner.innerHTML = `<p class="text-sm uppercase tracking-[0.2em] ${state.role === 'guest' ? 'text-pink-300' : 'text-emerald-300'}">${state.role === 'guest' ? 'Guest preview access' : 'Access granted'}</p><h3 class="mt-2 text-2xl font-bold text-white">${state.role === 'guest' ? 'Protected preview enabled' : 'You can open this video'}</h3><p class="mt-3 text-neutral-300">${state.role === 'guest' ? 'Guest playback includes a Hidden Gems watermark and download is disabled.' : 'VIP and admin can watch with full access. Download appears when a direct file is available.'}</p>`;
      if (source.type === 'file' && canWatchGuestPreview) {
        ...
        if (playableSrc) {
          const guestOverlay = state.role === 'guest' ? `<div class="pointer-events-none absolute inset-0 flex items-center justify-center"><img src="./assets/hidden-gems-logo.png" alt="Hidden Gems watermark" class="max-h-32 w-auto opacity-25 select-none" /><div class="absolute inset-0 bg-[repeating-linear-gradient(135deg,transparent,transparent_120px,rgba(255,255,255,0.04)_120px,rgba(255,255,255,0.04)_180px)]"></div></div>` : '';
          ...
          player.innerHTML = `<div class="space-y-4"><div class="relative overflow-hidden rounded-2xl border border-white/10 bg-black"><video ${state.role === 'guest' ? 'controls controlsList="nodownload noplaybackrate" disablepictureinpicture' : 'controls'} playsinline preload="metadata" class="w-full rounded-2xl bg-black"><source src="${escapeHtml(playableSrc)}" type="${escapeHtml(source.mimeType || 'video/mp4')}" />Your browser does not support embedded video playback for this file.</video>${guestOverlay}</div><div class="flex flex-wrap items-center justify-between gap-3">${source.fileName ? `<p class="text-sm text-neutral-500 break-all">${escapeHtml(source.fileName)}</p>` : '<span></span>'}${downloadButton}</div></div>`;
        } else {
        ...
        actionButton.textContent = state.role === 'guest' ? 'Watching preview' : 'Watching video'; actionButton.href = '#video-player-shell'; actionButton.target = '_self'; actionButton.rel = ''; actionButton.onclick = null;
      } else if (source.type === 'link') {
        if (state.role === 'guest') {
          player.innerHTML = '<p class="text-neutral-400">This video uses an external link. For protected guest previews with watermarking, upload the video file from the admin portal instead of using only a link.</p>';
  ...
  function renderAllVideosPage() {
    applyBg();
    document.body.innerHTML = shellHeader() + `<main class="mx-auto max-w-7xl px-6 py-14"><div class="rounded-[2rem] border border-pink-400/20 bg-gradient-to-br from-pink-500/10 via-fuchsia-500/10 to-transparent p-8"><p class="text-sm uppercase tracking-[0.25em] text-pink-300">All Videos</p><h1 class="mt-3 text-4xl font-black">Browse the full catalog</h1><p class="mt-4 max-w-2xl text-neutral-300">This page shows the videos your current account is allowed to see, with live sorting and filtering.</p></div><div id="all-videos-summary" class="mt-6 rounded-[1.5rem] border border-white/10 bg-white/5 px-5 py-4 text-sm text-neutral-300">Loading videos...</div><div class="mt-6 grid gap-4 lg:grid-cols-5"><label class="rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-neutral-300">Sort<select id="all-videos-sort" class="mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-white"><option value="newest">Newest</option><option value="oldest">Oldest</option><option value="title">Name</option><option value="price-low">Price: low to high</option><option value="price-high">Price: high to low</option></select></label><label class="rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-neutral-300">Category<select id="all-videos-category" class="mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-white"></select></label><label class="rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-neutral-300">Access<select id="all-videos-access" class="mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-white"><option value="all">All access</option><option value="guest">Guest</option><option value="vip">VIP</option></select></label><label class="rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-neutral-300 lg:col-span-2">Search<input id="all-videos-search" placeholder="Search by title or description" class="mt-2 w-full rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-white" /></label></div><div id="all-videos-grid" class="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-3"></div></main>` + shellFooter();
    bindCommonUi();
    const mount = async () => {
      await refreshSupabaseVideos(true);
      const state = await getState();
