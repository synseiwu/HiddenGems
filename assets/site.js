...
window.HiddenGemsApp = (() => {
  ...
  async function getPurchasedVideoIds(state = null) {
    // Direct PayPal Link mode: purchase unlocks are stored locally after the success redirect.
  ...
  function injectThemeStyles() {
    ...
    style.textContent = `
      ...
      html[data-hg-theme="light"] header, html[data-hg-theme="light"] footer { background: rgba(255,255,255,.86) !important; border-color: var(--hg-border) !important; color: var(--hg-text) !important; }
      ...
      html[data-hg-theme="light"] [class*="bg-white/5"], html[data-hg-theme="light"] [class*="bg-white/10"], html[data-hg-theme="light"] [class*="bg-white/["] { background-color: var(--hg-card) !important; }
      html[data-hg-theme="light"] [class*="border-white/"], html[data-hg-theme="light"] [class*="border-pink-400/"] { border-color: var(--hg-border) !important; }
      ...
      html[data-hg-theme="light"] input, html[data-hg-theme="light"] select, html[data-hg-theme="light"] textarea { background: rgba(255,255,255,.92) !important; color: #0f172a !important; border-color: var(--hg-border) !important; }
      html[data-hg-theme="light"] a:hover, html[data-hg-theme="light"] .text-pink-300, html[data-hg-theme="light"] .text-pink-400 { color: var(--hg-accent) !important; }
      html[data-hg-theme="midnight"] .bg-pink-500, html[data-hg-theme="midnight"] button.bg-pink-500, html[data-hg-theme="midnight"] a.bg-pink-500 { background-color: #2563eb !important; }
      html[data-hg-theme="midnight"] .text-pink-300, html[data-hg-theme="midnight"] .text-pink-400 { color: #93c5fd !important; }
  ...
  function applyBg() {
    ...
    document.body.className = theme === 'light' ? 'min-h-screen bg-slate-50 text-slate-950' : 'min-h-screen bg-black text-white';
    if (theme === 'light') document.body.style.background = 'radial-gradient(circle at top right, rgba(236,72,153,0.12), transparent 30%),radial-gradient(circle at left, rgba(217,70,239,0.08), transparent 24%),#f8fafc';
    else if (theme === 'midnight') document.body.style.background = 'radial-gradient(circle at top right, rgba(59,130,246,0.22), transparent 30%),radial-gradient(circle at left, rgba(168,85,247,0.14), transparent 24%),#020617';
    else document.body.style.background = 'radial-gradient(circle at top right, rgba(236,72,153,0.22), transparent 30%),radial-gradient(circle at left, rgba(217,70,239,0.14), transparent 24%),#000';
  }
      ...
      <header class="sticky top-0 z-50 border-b border-white/10 bg-black/75 backdrop-blur-xl">
        <div class="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <nav class="hidden gap-6 text-sm text-neutral-300 md:flex">
            ...
            <a data-nav-link="points-store" href="points-store.html" class="transition hover:text-pink-300">Access Info</a>
        ...
        <div id="mobile-menu" class="hidden border-t border-white/10 bg-black/95 px-6 py-4 md:hidden">
          <div class="flex flex-col gap-4 text-sm text-neutral-300">
            ...
            <a href="points-store.html">Access Info</a>
  ...
  async function finalizeStripeVideoCheckoutFromLocalStorage() {
    ...
    storage.addTransaction({ type: 'paypal-video', videoId, title: video.title || pending?.title || 'Purchased video', amountCents: 0 });
  ...
  function renderVideoPage() {
    ...
    const mount = async () => {
      ...
      if (!unlocked && state.role === 'guest' && video.access === 'guest') {
        banner.innerHTML = `<p class="text-sm uppercase tracking-[0.2em] text-pink-300">Guest Preview</p><h3 class="mt-2 text-2xl font-bold text-white">Preview before buying</h3><p class="mt-3 text-neutral-300">Guests can preview this title before purchase. Buy direct access using the PayPal link added for this video. Current price: <span class="font-semibold text-white">${escapeHtml(moneyLabelFromVideo(video))}</span>.</p>`;
        player.innerHTML = await renderPublicPreviewShell(video, `Preview only. Buy this title with its PayPal link to unlock full playback.`);
  ...
  function renderPointsStore() {
    ...
    const mount = async () => {
      ...
      else if (state.role === 'admin') banner.innerHTML = `Signed in: <span class="font-bold text-white">${state.email}</span> · Role: <span class="font-bold text-pink-300">admin</span> · Admin can manage PayPal links for each video.`;
  ...
  function renderAdminPage() {
    ...
    const mount = async () => {
      ...
      const renderVideoList = () => {
        ...
        if (!listEl) return;
        try {
        const videos = adminAllVideos().sort((a, b) => {
          const aTime = new Date(a.updatedAt || a.createdAt || 0).getTime() || 0;
          const bTime = new Date(b.updatedAt || b.createdAt || 0).getTime() || 0;
          return bTime - aTime;
        });
