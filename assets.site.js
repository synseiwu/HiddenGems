...
window.HiddenGemsApp = (() => {
  ...
  function canUseSupabaseFunctionInvoke() {
    const supabase = getSupabaseClient();
    if (!supabase?.functions?.invoke || !PAYPAL_FUNCTION_URL || !SUPABASE_URL) return false;
    try {
      const functionUrl = new URL(PAYPAL_FUNCTION_URL);
      const projectUrl = new URL(SUPABASE_URL);
      return functionUrl.origin === projectUrl.origin && /\/functions\/v1\/hg-paypal-checkout\/?$/.test(functionUrl.pathname);
    } catch (error) {
      return false;
    }
  }
  ...
  async function ensureCheckoutSession() {
    const supabase = getSupabaseClient();
    if (!supabase?.auth) return { accessToken: '' };
    let session = null;
    try {
      const sessionResult = await supabase.auth.getSession();
      session = sessionResult?.data?.session || null;
    } catch (error) {}
    if (!session?.access_token && supabase.auth.refreshSession) {
      try {
        const refreshed = await supabase.auth.refreshSession();
        session = refreshed?.data?.session || session;
      } catch (error) {}
    }
    if (!session?.access_token) throw new Error('Please sign in again before checkout so Hidden Gems can connect the payment to your account.');
  ...
  async function callPaypalPaymentFunction(payload) {
    ...
    if (canUseSupabaseFunctionInvoke()) {
      await ensureCheckoutSession();
      let invokeResult = await supabase.functions.invoke('hg-paypal-checkout', { body: payload });
      let invokeError = invokeResult?.error ? String(invokeResult.error.message || invokeResult.error || '').trim() : '';
      if (invokeError && /invalid jwt|jwt/i.test(invokeError) && supabase?.auth?.refreshSession) {
        try { await supabase.auth.refreshSession(); } catch (error) {}
        invokeResult = await supabase.functions.invoke('hg-paypal-checkout', { body: payload });
        invokeError = invokeResult?.error ? String(invokeResult.error.message || invokeResult.error || '').trim() : '';
      }
      if (invokeError) throw new Error(invokeError || 'PayPal checkout service error.');
      return invokeResult?.data || {};
    }

    let accessToken = await getSupabaseAccessToken();
    if (!accessToken) accessToken = (await ensureCheckoutSession()).accessToken;
  ...
  function renderLibraryPage() {
    ...
    const mount = async () => {
      ...
      document.getElementById('library-summary').innerHTML = `Role: <span class="font-bold text-pink-300">${state.role}</span> · Accessible titles: <span class="font-bold text-white">${videos.length}</span> · Wallet: <span class="font-bold text-white">${state.localPoints} points</span>`;
